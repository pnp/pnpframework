    ...
    public class XsltWebPartPostProcessorTests
    {
        ...
        public void Initialize()
        {
            using (var ctx = TestCommon.CreateClientContext())
            {
                ...
                template.Files.Add(new PnP.Framework.Provisioning.Model.File { Overwrite = true, Src = testPage, Folder = folder });
        ...
        private PnP.Framework.Utilities.WebParts.Schema.WebParts CreateBasicWebPart(string wpXml)
        {
            var serializer = new XmlSerializer(typeof(PnP.Framework.Utilities.WebParts.Schema.WebParts));
            using (var xmlStream = GetXmlStream(wpXml))
            using (var xmlReader = new XmlTextReader(xmlStream))
            {
                xmlReader.Namespaces = false;
                return (PnP.Framework.Utilities.WebParts.Schema.WebParts)serializer.Deserialize(xmlReader);
            }
        }

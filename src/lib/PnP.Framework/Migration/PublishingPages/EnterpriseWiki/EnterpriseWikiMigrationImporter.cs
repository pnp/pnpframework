...
namespace PnP.Framework.Migration.PublishingPages.EnterpriseWiki
{
    public sealed class EnterpriseWikiMigrationImporter
    {
            ...
            string approvedPlanDigest)
        {
            ...
            if (!PublishingPagePath.UriEquals(targetWeb.Url, package.Plan.TargetWebUrl))
            {
            ...
            var materialized = PageReferenceMaterializer.Materialize(
                ...
                package.Plan.DependencyActions);
            var rewrittenContent = PublishingPageContentTransformer.Rewrite(
            ...
            targetContext.ExecuteQueryRetry();
            var targetDirectory = PublishingPagePath.GetDirectoryName(package.Plan.TargetPageServerRelativeUrl);
            ...
            var targetFileName = PublishingPagePath.GetFileName(package.Plan.TargetPageServerRelativeUrl);
            ...
            foreach (var webPart in package.Snapshot.WebParts)
            {
                targetWeb.AddWebPartToWebPartPage(package.Plan.TargetPageServerRelativeUrl, new WebPartEntity
                {
                    ...
                    WebPartXml = PublishingPageContentTransformer.Rewrite(webPart.ExportXml, package.Plan.Replacements)
                });
            }

            ...
            IEnumerable<PageTextReplacement> replacements)
        {
            switch (field.Kind)
            {
                case PageFieldValueKind.String:
                    targetItem[targetInternalName] = PublishingPageContentTransformer.Rewrite(field.Value, replacements);
                    break;
                ...
                case PageFieldValueKind.Url:
                    ...
                    {
                        Url = PublishingPageContentTransformer.Rewrite(field.UrlValue?.Url, replacements),

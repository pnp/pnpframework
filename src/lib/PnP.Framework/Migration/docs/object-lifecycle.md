...
| List/library | Site/Web/List IDs, title/path, template, settings | Create owned, reuse owned, or block | Web/path/title collision, template, ownership, permissions | Source List ID -> target List ID and actual disposition | Identity, settings, ownership, full supported closure |
...
| Publishing Page | Identity, content, layout, lifecycle, fields, Web Parts | Create page or block | Pages library, target content type/layout, page collision | Target file ID, List item ID, version, content type | Content digest, fields, Web Parts, CT, lifecycle |
...
`TopologyPlan` maps the source hierarchy to deterministic target Site/Web locations. `TopologyTargetAnalysis` records one probe for every mapped Web and selects:
...
- `Block` for missing required hosts, unowned collisions, parent/template mismatch, or insufficient evidence.
...
- `Block`: required schema cannot be safely satisfied.
...
Plans select `CreateOwned`, `ReuseOwned`, or `Block`. Target probes record parent availability, existing identity and metadata, FieldLinks, fields, collisions, permissions, and diagnostics. Execution orders ancestors before descendants and verifies parent, metadata flags, and required FieldLinks after materialization.
...
A `ListMaterializationPlan` selects `CreateOwned`, `ReuseOwned`, or `Block`, plus nested field, View, and site-content-type plans. `ListTargetProbe` records whether the target Web exists or is deferred behind topology, path/title collisions, target IDs, template, ownership markers, manageability, disposition, and typed issues.
...
Lists execute in lookup topological order. The importer materializes site CT closure, List identity/settings, fields, List CTs and order, folders/items/files/attachments, and Views. It records target List, item, View, and List-local CT IDs for downstream lookup values and Web Part rebinding.

Fresh verification checks the supported List closure and exact current file/attachment bytes. A List mismatch makes `listsMatched=false` even when the page was created.
